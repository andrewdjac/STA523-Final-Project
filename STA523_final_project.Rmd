...
players_names <- readRDS("data/player_names.rds")
```
  ...
  g <- players_data %>% 
    ...
    geom_hline(yintercept = 
                 mean(players_data %>% filter(name == p, category == c) %>%
                       .$quantity), col = "#4169e1") +
    labs(x = "Date", y = c %>% toTitleCase(),size=4) +
...
navbarPage(
      title = "Duke Men's Basketball ",
      id = 'nav',
      tabPanel('Application', value = 'Application ',
                 fluidRow(
...
src=https://media.giphy.com/media/AFXYakZdPOEkxVqNyU/giphy.gif width = 40% height = 100%>'))),
      ...
      column(12, h4("Geo-Location"), 
                        leafletOutput("leaf", width = "100%", height = 100),
...
server <- function(input, output, session){
  #Getgeo
      GetGeo <- reactive(
      players_names %>% filter(Name == input$select_player) %>% select(lat, lon) %>% unique()
   ...
   output$leaf = renderLeaflet({
      with(GetGeo(),
          leaflet(data = GetGeo()) %>%
            ...
            addTiles() %>%
            addMarkers(~lon, ~lat, popup = input$select_player)
           )
    })
